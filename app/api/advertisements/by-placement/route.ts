...
// GET - Get advertisement by placement
  ...
  try {
    ...
    if (!placement) {
      return NextResponse.json(
        { success: false, error: 'Placement parameter is required' },
        { status: 400 }
      );
    }
    ...
    const [advertisements] = await pool.query<Advertisement[]>(query, [placement]);

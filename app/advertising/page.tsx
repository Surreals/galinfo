...
export default function AdvertisingPage() {
  ...
  const breadcrumbs = [
    ...
    { name: 'Замовити рекламу', item: 'https://galinfo.com.ua/advertising' }
  ];
    ...
    const fetchTemplateData = async () => {
      try {
        ...
        if (result.success && result.data) {
          const template = result.data.find((template: any) => template.template_id === 'advertising-page');
          if (template) {
  ...
  if (!pageData) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-6">ЗАМОВИТИ РЕКЛАМУ</h1>

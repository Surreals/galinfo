...
function get_rsshome(){
	...
	$ttop.="<a href='".$depot['vars']['language_link']."rss/export.xml' class=rssl>http://".$_SERVER['HTTP_HOST']."/".$par['lng']."/rss/export.xml</a>";
	...
	foreach ($topro as $ct){
		if (isset($haza[$ct])){
			...
			foreach ($haza[$ct] as $k=>$v){
				...
				$ttop.="<a href='".$depot['vars']['language_link']."rss/".$ct."/".$v[0].".xml' class=rssl>http://".$_SERVER['HTTP_HOST']."/".$depot['vars']['language_link']."rss/".$ct."/".$v[0].".xml</a>";
			}
		}
	}
...
function complete_rss($arr){
	...
	$arr['rssurl'] = 'http://'.$_SERVER['HTTP_HOST'].'/';
	$arr['rssimage'] = 'http://'.$_SERVER['HTTP_HOST'].'/im/logo-rss-100.png';
	$arr['rssimage180'] = 'http://'.$_SERVER['HTTP_HOST'].'/im/logo-rss-180x.png';
...
function get_sitemap(){
	...
	foreach ($sql as $res){
		...
		$arr['items'][]=array(
				"link"		=>	"http://".$_SERVER['HTTP_HOST']."/".$subpath.$res['param']."/",
				"lastmod"	=>	$now,
				"frequency"	=>	"hourly",
				"priority"	=>	0.7
		);
	...
	foreach (explode("|","news|announces|blogs|video|audio|photo|articles|ljlive|mainmedia") as $ntype){
		$arr['items'][]=array(
				"link"		=>	"http://".$_SERVER['HTTP_HOST']."/$ntype/",
				"lastmod"	=>	$now,
				"frequency"	=>	"hourly",
				"priority"	=>	0.7
		);

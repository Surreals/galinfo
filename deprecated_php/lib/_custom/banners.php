...
function getbanner($plid){
	...
	if (!mysql_num_rows($banners_run)) return;
	$opentag=$closetag='';
	

	/*echo mysql_num_rows($banners_run)."<br>";*/
	if (mysql_num_rows($banners_run)==1){
		$current_banner=mysql_fetch_assoc($banners_run);
	} else {
		$in=0;
		$min=0;
		while ($res=mysql_fetch_assoc($banners_run)) {

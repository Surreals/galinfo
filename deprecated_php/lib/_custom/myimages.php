...
function myimagesRemove(){
	...
	if (isset($par['godelete'])) {
		...
		foreach (explode(",",$par['imageids']) as $id){
			...
			$pic=mysql_fetch_assoc($sql_count_text);
			if ($pic['filename']){
				sqlquery("DELETE FROM ".PICS." WHERE id = \"".sqller($id)."\" AND userid = \"".sqller($depot['logged_user']['usid'])."\"");
...
function myimagesSave(){
	...
	if ($par['a']=='savenew'){
		...
		$query ="INSERT INTO ".PICS." SET filename=\"".mysql_escape_string(stripslashes($depot['savedimage'][0]))."\",";
	}
	else {
		if (isset($_FILES[$name])&&($_FILES[$name]['name']!='')) {
			...
			$query ="UPDATE	".PICS." SET filename=\"".mysql_escape_string(stripslashes($depot['savedimage'][0]))."\",";
		} else {
	...
	/*$par['imageid'] = mysql_insert_id();
	mysql_query("UPDATE ".PICS." SET orderid = ".mysql_insert_id()." WHERE id = ".mysql_insert_id()."");*/
...
function uploadAndSave($name){
		...
		if (isset($_FILES[$name])&&($_FILES[$name]['name']!='')){
			...
			if (is_uploaded_file($_FILES[$name]['tmp_name'])) {
					...
					$ishere=mysql_fetch_row(mysql_query("SELECT COUNT(*) FROM ".PICS." WHERE filename=\"$fname\""));

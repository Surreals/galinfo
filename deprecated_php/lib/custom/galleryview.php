...
function galleryViewerOld($images){
	global $depot;
	...
	foreach ($images as $im){
		...
		if ($i==1){
			...
			$depot['vars']['ogimage']="<meta property=\"og:image\" content=\"https://".$_SERVER['HTTP_HOST']."/media/gallery/full/".$im['filename']."\" />";
			$ogimage=true;
...
function galleryViewer($images, $count_in_line=8){
	global $depot;
	$gallery_id = uniqid();
	if (!count($images)) return;
	$depot['vars']['ogimage']="<meta property=\"og:image\" content=\"https://".$_SERVER['HTTP_HOST']."/media/gallery/full/".$images[0]['filename']."\" />";



	$tpl = dirname(dirname(__FILE__)).'/tpl/news/slick.tpl';
	ob_start();
	include $tpl;

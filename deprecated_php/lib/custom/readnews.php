...
function get_newsread(){
	...
	if (isset($par['newsid'])) {
		...
		if ($par['articletype']=='blogs'){
			...
			if ($_SERVER['HTTP_HOST']!==$depot["vars"]['domain']){
				var_dump($depot["vars"]['domain'].$_SERVER['REQUEST_URI']);die;
				return;
			}
		}
		...
		$sql_n= "	
			SELECT	".NEWS.".*,
					...
					".NEWSMETA.".nkeywords,
					".USERS.".uname_ua,
					".CATS.".description as cdescription
			FROM		".NEWS." 
			LEFT JOIN	".NEWSB."		USING	(id)
			LEFT JOIN	".NEWSHEAD."	USING	(id)
			LEFT JOIN	".NEWSMETA."	USING	(id)
			LEFT JOIN   ".CATS." ON ".CATS.".id = ".NEWS.".region
			LEFT JOIN	".USERS." ON ".USERS.".id = ".NEWS.".nauthor
			WHERE		
		...
		//echo $sql_n."<hr>";
		$depot['current_article'] = conn_fetch_assoc($the_news);
	...
	$tags = getTagsWords($depot['current_article']['id']);
	if($tags){
        $tags = ', '.getTagsWords($depot['current_article']['id']);
    }

    $keywords = extractKeywords($depot['current_article']);

	if($keywords){
	    $keywords = ', '.extractKeywords($depot['current_article']);
    }

    $region_description = $depot['current_article']['cdescription'];
	...
	$dtitle			=	(trim($depot['current_article']['ntitle'])) ? $depot['current_article']['ntitle']: ucfirst($depot['lxs']['news']) .$region_description.  $depot['current_article']['nheader'];
	$ddescription	=	(trim($depot['current_article']['ndescription'])) ? $depot['current_article']['ndescription']:$depot['current_article']['nteaser'];
	$dkeywords		=	(trim($depot['current_article']['nkeywords'])) ? $depot['current_article']['nkeywords'] : mb_strtolower($depot['lxs']['news'],"UTF-8").$tags .$keywords;


	$depot['vars']['title']			=	cleanMeta(getfromsql($dtitle,$par['lng']));
	$depot['vars']['ogtitle']		=	"<meta property=\"og:title\" content=\"".htmlspecialchars(getfromsql($dtitle,$par['lng']))."\" />";
	$depot['vars']['ogtype'] = '<meta property="og:type" content="article" >';
	...
	$depot['vars']['showauthor'] = $depot['current_article']['showauthor'];
	$depot['vars']['author_name'] = $depot['current_article']['uname_ua'];
...
function readnews(){
	...
	$depot['vars']['_js'] = '<script type="text/javascript" src="/js/slick.js"></script>';
	$depot['vars']['_css'] = '<link rel="stylesheet" type="text/css" href="/css/slick.css">';
	...
	if ($depot['current_article']['images'] && $depot['current_article']['layout'] !=='10') {
		...
		if (count($images)) {
			if($pattern=='readSlider'){
				$used_images =[];
				$gallery_codes = [];
				preg_match_all('/\<.*\>\{\{(.*)\}\}\<\/.*\>/', $body, $gallery_codes);

				foreach($gallery_codes[0] as $code){
					$code_ids = trim(strip_tags($code),'{}');
					$code_ids = $code_ids?explode(',',$code_ids):[];
					$code_images = [];
					foreach($code_ids as $item_id){
						if(isset($images[$item_id-1])){
							$code_images[]=$images[$item_id-1];
							$used_images[] = $item_id-1;
						}
					}
					require_once("galleryview.php");
					$code_gall = galleryViewer($code_images,5);
					$body = str_replace($code,$code_gall,$body);
				}
				foreach($used_images as $used_image){
					unset($images[$used_image]);
				}
	...
	if ($depot['current_article']['showauthor']){
			$auth="<div class=\"brick author\">".$depot['current_article']['uname_ua'].", Гал-інфо.</div>";
	} else $auth='';

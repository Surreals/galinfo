...
function get_gallery(){
	...
	//echo $from."---".$to."---".$count[0]."---".conn_sql_num_rows($sql_1_run)."<br>".$sql_1;
	
	$imagelist='';
	
	for ($i=0;$i<conn_sql_num_rows($sql_1_run);$i++){
		$res=conn_fetch_assoc($sql_1_run);
		if ($i==0){
			if ($res['raw']) {

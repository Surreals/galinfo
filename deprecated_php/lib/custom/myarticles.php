...
function myCivilAdd(){
	...
	$sql_themes=conn_sql_query($sql_themes_text);
	while ($res_1 = conn_fetch_assoc($sql_themes)){
...
function myCivilSave(){
    ...
    $res_ = conn_sql_query($sql) or $depot['errors'][]= conn_error();
	$no=0;
	...
	if($par['a'] == 'savenew'){
		...
		conn_sql_query($sql2);
		conn_sql_query($sql3) or die(conn_error());
		$depot['oks'][]=$depot['lxs']['changesaved1'];
	}

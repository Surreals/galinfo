	...
	if (count($run_sql_main)){
		...
		foreach ($run_sql_main as $item){
			...
			$arr['items'][]=array(
				...
				"allnewslink"	=>	"https://".$depot['vars']['subdomain1'].$depot['vars']['language_link']."blogs/",
				"boxtitle"		=>	mb_strtoupper($depot['lxs']['blogs'],"utf8"),
			
				"bloggerlink"	=>	"https://".$depot['vars']['subdomain1'].$depot['vars']['language_link']."bloggers/".$item['uuserid']."/",
...
function anaheads(){
	...
	$arr=array();
		...
		/*while ($res=conn_fetch_assoc($run_sql)){*/
...
function topread($limit=5,$iscollection=NULL,$pattern="justlist"){
    ...
    $limit = isset($depot['enviro']['qty_read_now']) ? (int)$depot['enviro']['qty_read_now']  : $limit;
	...
	foreach ($sql as $v){
		...
		if ($v['images'])	{
			...
			if (count($images)) {
			 ...
			 $image = "<img src='https://img.youtube.com/vi/".$v['youcode']."/hqdefault.jpg'>";
		}
...
function socials($news){
	global $lxs,$depot;
	$fb="https://www.facebook.com/sharer.php?u=https://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']."&t=".urlencode($news['nheader']);
	$tw="https://twitter.com/home/?status=".trim($news['nheader'])." https://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'];
	$lj="https://www.livejournal.com/update.bml?subject=".urlencode($news['nheader'])."&event=https://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'];
...
function newsCollection ($params){
	...
	if (is_array($params['extraConditions'])) {
		...
		if (isset($params['extraConditions']) && $params['extraConditions'] != '')

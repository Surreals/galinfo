	...
	foreach ($par as $k=>$v){
		...
		if (@$matches[0]){
			if ($par[$matches[0]]) {
				...
				conn_sql_query($query);

				if (conn_error()) $depot['errors'][]= conn_error()."<br><br><br>$query";

			}
		}
	}

	conn_sql_query("DELETE FROM ".PICSU." WHERE tempid=\"".$par['uniquetemp']."\" AND userid=0");

	if (conn_error())$depot['errors'][]=$depot['lxs']['problemsaving']."<hr>".conn_error();  else $depot['oks'][]=$depot['lxs']['chanessaved'];
...
function myDetailsView(){
	...
	$res=conn_fetch_assoc(conn_sql_query($sql_text));
	...
	/*FOR image*/
	$imagesSQL=conn_sql_query("SELECT * FROM ".PICSU." WHERE userid=\"".sqller($depot['logged_user']['usid'])."\"");
	if (conn_sql_num_rows($imagesSQL)>0){
		$resimage=conn_fetch_assoc($imagesSQL);
	} else {
		$resimage=array();
	}

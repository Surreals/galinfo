...
function check_form(){
	...
	$dodocount=mysql_fetch_row(mysql_query("SELECT COUNT(*) FROM ".FUSERS." WHERE humancheck = \"".$par['humancheck']."\""));
	if ($dodocount[0]) {
...
function reg_save(){
	...
	";
	sqlquery($sql) or die($sql."<br><br>".mysql_error());
...
function reg_success(){
	...
	if (!quick_mail($par['email'],$subj,$body,$depot['enviro']['daemon_name'],$depot['enviro']['daemon_address'])){
		...
		sqlquery("DELETE FROM ".FUSERS." WHERE id = \"".mysql_insert_id()."\"");
		return reg_form();
	} else {
...
function get_confirm(){
	...
	if (!@$par['key']) {
		...
		mysql_query("UPDATE ".FUSERS." SET approved = 1, uniqueid=NULL WHERE uniqueid = \"".sqller($par['key'])."\" AND approved=0");

...
// Images Directory
imagesDir = "/var/wwigs/icons/";

// CSS Directory
cssDir = "/var/wwigs/styles/";

// Popups Directory
popupsDir = "/var/wwigs/popups/";
...
BlockFormats[0]  = new Array("Абзац","<p>");
BlockFormats[1]  = new Array("Заголовок H1","<h1>");
BlockFormats[2]  = new Array("Заголовок H2","<h2>");
BlockFormats[3]  = new Array("Заголовок H3","<h3>");
BlockFormats[4]  = new Array("Заголовок H4","<h4>");
...
buttonName2[17]  = "undo";
buttonName2[18]  = "redo"; 
buttonName2[19]  = "seperator";*/
buttonName2[10]  = "inserttable";
buttonName2[11]  = "insertimage";

buttonName2[14]  = "seperator";

buttonName2[21]  = "removeformat";
buttonName2[22]  = "viewSource";
;
	
// List of available actions and their respective ID and images
var ToolbarList = {
//Name              buttonID                 buttonTitle           buttonImage                            buttonImageRollover
  "bold":           ['bold',                 'Жирний',               imagesDir + 'bold.gif',               imagesDir + 'bold_on.gif'],
  "italic":         ['Italic',               'Курсив',             imagesDir + 'italics.gif',            imagesDir + 'italics_on.gif'],
  "underline":      ['Underline',            'Підкреслений',          imagesDir + 'underline.gif',          imagesDir + 'underline_on.gif'],
	"strikethrough":  ['Strikethrough',        'Перекреслений',      imagesDir + 'strikethrough.gif',      imagesDir + 'strikethrough_on.gif'],
	"seperator":      ['',                     '',                   imagesDir + 'seperator.gif',          imagesDir + 'seperator.gif'],
	"subscript":      ['Subscript',            'Нижній індекс',          imagesDir + 'subscript.gif',          imagesDir + 'subscript_on.gif'],
	"superscript":    ['Superscript',          'Верхній індекс',        imagesDir + 'superscript.gif',        imagesDir + 'superscript_on.gif'],
	"unorderedlist":  ['InsertUnorderedList',  'Ненумерований список',imagesDir + 'list_unordered.gif',     imagesDir + 'list_unordered_on.gif'],
	"orderedlist":    ['InsertOrderedList',    'Нумерований список',  imagesDir + 'list_ordered.gif',       imagesDir + 'list_ordered_on.gif'],
	"outdent":        ['Outdent',              'Відступ вліво',            imagesDir + 'indent_left.gif',        imagesDir + 'indent_left_on.gif'],
	"indent":         ['Indent',               'Відступ вправо',             imagesDir + 'indent_right.gif',       imagesDir + 'indent_right_on.gif'],
	"forecolor":      ['ForeColor',            'Колір тексту',          imagesDir + 'forecolor.gif',          imagesDir + 'forecolor_on.gif'],
	"backcolor":      ['BackColor',            'Колір фону',          imagesDir + 'backcolor.gif',          imagesDir + 'backcolor_on.gif'],
	"undo":           ['Undo',                 'Відмінити дію',               imagesDir + 'undo.gif',               imagesDir + 'undo_on.gif'],
	"redo":           ['Redo',                 'Повторити відмінену дію',               imagesDir + 'redo.gif',               imagesDir + 'redo_on.gif'],
	"inserttable":    ['InsertTable',          'Вставити таблицю',        imagesDir + 'insert_table.gif',       imagesDir + 'insert_table_on.gif'],
	"insertimage":    ['InsertImage',          'Вставити зображення',        imagesDir + 'insert_picture.gif',     imagesDir + 'insert_picture_on.gif'],
	"createlink":     ['CreateLink',           'Створити лінк',         imagesDir + 'insert_hyperlink.gif',   imagesDir + 'insert_hyperlink_on.gif'],
	 "unlink":			['Unlink',				'Скасувати лінк',         imagesDir + 'unlink.gif',   imagesDir + 'unlink_on.gif'],
	"viewSource":     ['ViewSource',           'Переглянути код',         imagesDir + 'view_source.gif',        imagesDir + 'view_source_on.gif'],
	"viewText":       ['ViewText',             'Переглянути дизайн',           imagesDir + 'view_text.gif',          imagesDir + 'view_text_on.gif'],
	 "selectformat":     ['SelectFormat',       'Форматувати абзац',         imagesDir + 'select_format.gif',        imagesDir + 'select_format_on.gif'],
	  "removeformat":   ['RemoveFormat',         'Очистити форматування WORD',    	imagesDir + 'remove_format.gif',      imagesDir + 'remove_format_on.gif']
};
...
\* ---------------------------------------------------------------------- */
function generate_wysiwyg(textareaID,height) {
	if (height)
	{
  ...
  var toolbar;
  toolbar =  '<table cellpadding="0" cellspacing="0" border="0" class="toolbar1" style="width:100%;border-top:#999 solid 5px;border-collapse:separate;"><tr><tbody id="barm"><td style="width: 6px;"><img src="' +imagesDir+ 'seperator2.gif" alt="" hspace="3"></td>';
  
	// Create IDs for inserting Font Type and Size drop downs
	//toolbar += '<td style="width: 90px;"><span id="FontSelect' + n + '"></span></td>';
	toolbar += '<td style="width: 100px;"><span id="FontFormats'  + n + '"></span></td>';
	//toolbar += '<td style="width: 100px;"><span id="FontSizes'  + n + '"></span></td>';
 ...
 var sourceButton='';
  ...
  for (var j = 0; j <= buttonName2.length;) {
    if (buttonName2[j]) {
		  ...
		  if (buttonName2[j] == "seperator") {
			...
			sourceButton += '<span id="HTMLMode' + n + '"><img src="'  +buttonImage+  '" border=0 unselectable="on" title="' +buttonTitle+ '" id="' +buttonID+ '" class="button1" onClick="formatText(this.id,\'' + n + '\');" onmouseover="if(className==\'button1\'){className=\'buttonOver1\'}; this.src=\'' +buttonImageRollover+ '\';" onmouseout="if(className==\'buttonOver1\'){className=\'button1\'}; this.src=\'' + buttonImage + '\';" unselectable="on"  width="80" height="20"></span>';
			sourceButton += '<span id="textMode' + n + '"><img src="' +imagesDir+ 'view_text.gif" border=0 unselectable="on" title="Режим дизайну"          id="ViewText"       class="button1" onClick="formatText(this.id,\'' + n + '\');" onmouseover="if(className==\'button1\'){className=\'buttonOver1\'}; this.src=\'' +imagesDir+ 'view_text_on.gif\';"    onmouseout="if(className==\'buttonOver1\'){className=\'button1\'}; this.src=\'' +imagesDir+ 'view_text.gif\';" unselectable="on"  width="80" height="20"></span>';
			}
	    else {
  ...
  toolbar +=toolbar2 + '<td>&nbsp;</td></tbody></tr></table>';  
  toolbar2='';
	...
	var iframe = '<table cellpadding="0" cellspacing="0" border="0" style="width:100%; height:' + wysiwygHeight + 'px;border: 1px inset #CCCCCC;background:#FFF;"><tr><td valign="top">\n'
  + '<iframe frameborder="0" id="wysiwyg' + n + '" class=tools></iframe>\n'
  + '</td></tr></table>\n';

  iframe +='<table cellpadding="0" cellspacing="0" border="0" class="toolbar1" style="width:100%"><tr><td>'+sourceButton+"</td></tr></table>";
  ...
  var content1 = document.getElementById(n).value;

	var content = "<html>\n<head>\n";
	if (cssFile) content += "<link media=\"all\" type=\"text/css\" rel=\"stylesheet\" href=\""+cssFile+"\" >\n";
	content += '</head>\n<body class="read" style="text-align:left;">'+content1+'</body>\n</html>';
  ...
  if (browserName == "Microsoft Internet Explorer") {
    for (var idx=0; idx < document.forms.length; idx++) {
  	...
  	for (var idx=0; idx < document.forms.length; idx++) {
    	document.forms[idx].addEventListener('submit',function OnSSubmit() { updateTextArea(n); }, false);
    }
  }	 
...
function formatText(id, n, selected) {
  ...
  document.getElementById("wysiwyg" + n).contentWindow.focus();
	...
	if (viewTextMode == 1 && disabled_id == 1) {
	  alert ("Ви перебуваєте в режимі HTML. Переключітья в режим дизайну.");	
	}
      ...
      var currentColor = _dec_to_rgb(document.getElementById("wysiwyg" + n).contentWindow.document.queryCommandValue(id));
		...
		if (window.popupIMAGES == undefined || popupIMAGES.closed){
			popupIMAGES = window.open(popupsDir + 'insert_image.html?wysiwyg=' + n,'popup','location=0,status=0,scrollbars=0,resizable=0,width=600,height=350');
		}
		else 
		popupIMAGES.focus();
...
function outputFontFormats(n) {
  var FontFormatObj        = ToolbarList['selectformat'];
  var FontFormat			= FontFormatObj[2];
  var FontFormatOn         = FontFormatObj[3];

	BlockFormats.sort();
	var FontFormatsDropDown = new Array;
	FontFormatsDropDown[n] = '<table border="0" cellpadding="0" cellspacing="0"><tr><td onMouseOver="document.getElementById(\'selectFormat' + n + '\').src=\'' + FontFormatOn + '\';" onMouseOut="document.getElementById(\'selectFormat' + n + '\').src=\'' + FontFormat + '\';"><img src="' + FontFormat + '" id="selectFormat' + n + '" width="100" height="20" onClick="showFontFormats(\'' + n + '\');" unselectable="on"><br>';
	FontFormatsDropDown[n] += '<span id="Formats' + n + '" class="dropdown" style="width: 140px;">';

	for (var i = 0; i <= BlockFormats.length;) {
		if (BlockFormats[i]) {
			FontFormatsDropDown[n] += '<button type="button" onClick="formatText(\'FontFormat\',\'' + n + '\',\'' + BlockFormats[i][1] + '\')\;hideFontFormats(\'' + n + '\');" onMouseOver="this.className=\'mouseOver\'" onMouseOut="this.className=\'mouseOut\'" class="mouseOut" style="width: 100px;"><table cellpadding="0" cellspacing="0" border="0"><tr><td align="left"><font size="2">' + BlockFormats[i][0] + '</td></tr></table></button><br>';	
		}	  
		i++;
	}
	
	FontFormatsDropDown[n] += '<button type="button" onClick="hideFontFormats(\'' + n + '\');" onMouseOver="this.className=\'mouseOver\'" onMouseOut="this.className=\'mouseOut\'" class="mouseOut" style="width: 100px;"><table cellpadding="0" cellspacing="0" border="0"><tr><td align="left"><font size="2">* * * * * * </td></tr></table></button><br>';
	FontFormatsDropDown[n] += '</span></td></tr></table>';
	document.getElementById('FontFormats' + n).insertAdjacentHTML("afterBegin", FontFormatsDropDown[n]);
};
...
function hideFontFormats(n) {
  document.getElementById('Formats' + n).style.display = 'none'; 
};
...
function showFontFormats(n) { 
  if (document.getElementById('Formats' + n).style.display == 'block') {
    document.getElementById('Formats' + n).style.display = 'none';
	}
  else {
    document.getElementById('Formats' + n).style.display = 'block'; 
    document.getElementById('Formats' + n).style.position = 'absolute';		
  }
};
...
function viewSource(n) {
  ...
  var browserName = navigator.appName;
	var menubar = document.getElementById("barm");
	var buttons = document.getElementsByName("addne");
...
function removeFormat(n) {
	...
	str = str.replace(/<xml>([\w\W\d\D\s\S]*)<\/xml>/gi, '');
	str = str.replace(/<style>([\w\W\d\D\s\S]*)<\/style>/gi, '');
	str = str.replace(/<!([^><]+)>/gi,'');

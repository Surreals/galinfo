...
function aget_center(){
	...
	switch ($par['su']){
		case "v":
		case "changeenviro":$ttop.= propsheadline_enviro();break;
		case "add": $ttop.= propsheadline_add();break;
		case "remove": $ttop.= propsheadline_del();break;
	}
...
function propsheadline_enviro(){
	...
	if ($par['su'] == "changeenviro"){
		...
		$res_ = conn_sql_query("UPDATE ".ENVIROHEADLINE." SET envalue = \"".$par[$par['arrayname']]."\" WHERE envid = \"$v\"");
		if (conn_affected_rows($res_)>0){
			$depot['oks'][]=iho("Значення параметру збережено");
			freecache(6);
			$enviro=conn_fetch_array(conn_sql_query("SELECT * FROM ".ENVIROHEADLINE.""));
		}
	}
	...
	<table width=100% cellpadding=5><tr>
		<td class=heaad width=40%>".iho('Призначення')."</td>
		<td class=heaad width=20%>".iho('Параматер')."</td>
		<td class=heaad width=20%>".iho('Значення')."</td>
		<td class=heaad width=20%>".iho('Операції')."</td></tr>";

	$esql=conn_sql_query("SELECT * FROM ".ENVIROHEADLINE." ORDER BY title") or die(conn_error());
	...
	$ttop.="</table>
		<input type=hidden name=act value=\"propsheadline\"><input type=hidden name=su value=\"\"><input type=hidden name=arrayname value=\"\"><input type=hidden name=id value=\"\">";
	return $ttop;
}


function propsheadline_add(){
	...
	if (!isset($par['sudir'])){
		...
		for ($i=0;$i<$par['qty'];$i++){
			/*-----------------*/
			$ison=conn_fetch_row(conn_sql_query("SELECT * FROM ".ENVIROHEADLINE." WHERE envid=\"".conn_real_escape_string(trim($par["idword$i"]))."\""));
			if ($ison[0]){
		...
		for ($i=0;$i<$par['qty'];$i++){
			...
			$q21=conn_sql_query("INSERT INTO ".ENVIROHEADLINE." SET 
	...
	$ttop.= "<input type=hidden name=act value=\"propsheadline\">
...
function propsheadline_del(){
	global $par,$depot,$oks,$b;
	$ttop='';

    $res_ = conn_sql_query("DELETE FROM ".ENVIROHEADLINE." WHERE id=\"".$par['id']."\"");
...
function pic_js(){
 ...
 <script language="javaScript">
		...
		function editenv(pattidvar){
			...
			objScript.src = "/gazda/adm_get_list.php?pattidvar="+pattidvar+"&upd=enviroheadline&timest="+Math.random();

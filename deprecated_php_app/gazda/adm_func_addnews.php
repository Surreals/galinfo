...
function news_add(){
	...
	if ($par['su'] == 'edit') {
		...
		if (is_array($blocked) && count($blocked)>1){
				...
				<td valign=top>
					...
					<div id='headholder1' class='tabholder'>
						<div style='float:left;width:99%'>
							<label for=nheader class=mn>Заголовок(<span></span>)</label>".bd_tar(@$par['nheader'],'nheader','99%','45px',1)."
						</div>
						...
						<label for=nteaser class=mn>Лід(<span></span>)</label>".bd_tar(@$par['nteaser'],'nteaser','99%;','80px',2)."
					</div>
	...
	$html.="";
	$sql='SELECT id, title, cattype, description FROM '.CATS.' WHERE lng="'.$lngs1[$par["lang"]][1].'" ORDER BY orderid';
	...
	for ($i=0;$i<conn_sql_num_rows($query);$i++) {
		...
		switch ($res[2]) {
			case 3	:	$regs[]=$res['0'];$regs[]=$res['1'];$descr[$res['0']]=$res['3'];break;
	...
	$html.="
			...
			<div style='width:48%;float:right;'>
				...
				<fieldset style=''><legend>Регіон</legend>
					".bd_multipopup($regs,'region','height:85px;width:100%;font-size:11px;overflow:auto;',5,'',$descr)."
				</fieldset>
...
function toTwitter($news_id){
	...
	if($article){
		require_once("../lib/third_party/twitter/codebird.php");
...
function news_view(){
	...
	$html.="</td>
				...
				".bd_popup($usrs,'news_author','width:99%;font-size:11px;',3,'')."
				</td>
</tr>";
	...
	$html.="
							...
							<td width=60>
								<input type=submit name=generate_file id='submt' value='Експорт новин'>
							</td>
						</tr>
					</table>
				</tr>
	...
	if(@$par['news_author']){
		$addsql .= " AND nauthor=".$par['news_author'];
		$path.="&nauthor=".$par['news_author'];
	}
	...
	if(isset($_POST['generate_file'])){
		downloadExport(str_replace($limit,'',$sql1),'news.csv');
	}
